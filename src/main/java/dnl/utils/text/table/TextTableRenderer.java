...
public class TextTableRenderer implements TableRenderer {
    ...
    public void render(OutputStream os, int indent) {
        ...
        String headerStartSep = StringUtils.repeat("_",
                totLength + tableModel.getColumnCount() * 2);
        ps.print(indentStr);
        ...
        indentAccordingToNumbering(ps, indexFormat1);
        String headerSep = StringUtils.repeat("=",
                totLength + tableModel.getColumnCount() * 2 - 1);
        ps.print(indentStr);
        ps.print("|");
        ps.print(headerSep);
        ps.println("|");
    ...
    private String resolveSeparator(int[] lengths) {
        ...
        for (int j = 0; j < tableModel.getColumnCount(); j++) {
            ...
            lengths[j] = Math.max(tableModel.getColumnName(j).length(),
                    lengths[j]);
    ...
    private void addSeparatorIfNeeded(PrintStream ps, String separator,
            String indexFormat1, int i, String indentStr) {
        if (!textTable.separatorPolicies.isEmpty()
                && textTable.hasSeparatorAt(i)
