                    ...
                    .getHandler().getAnnotationClass()))
            {
                if (event.getClass() == RequestEvent.class
                        && RequestEvent.class == trigger.getMethod()
                                .getParameterTypes()[0]
                        && trigger.getEventType() == Type.WEBSOCKET) {
                    if (event.getTrigger() != null
                            && event.getTrigger().length > 0
                            && trigger.getTrigger().length > 0) {
                        if (trigger.getTrigger()[0].equalsIgnoreCase(event
                                .getTrigger()[0])
                                && trigger.getTrigger()[1]
                                        .equalsIgnoreCase(event.getTrigger()[1])) {
                            if (trigger.getMethod().getParameterTypes()[0]
                                    .isInstance(event)) {
                                try {
                                    trigger.getMethod().invoke(
                                            trigger.getHostClass()
                                                    .newInstance(), event);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                        ...
                        if (trigger.getMethod().getParameterTypes()[0]
                            ...
                            try {
                                trigger.getMethod().invoke(
                                        trigger.getHostClass().newInstance(),
                                        event);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                ...
                } else if (event.getClass() == WebEvent.class
                        && WebEvent.class == trigger.getMethod()
                                .getParameterTypes()[0]
                        && trigger.getEventType() == Type.HTTP) {
                    if (event.getTrigger() != null
                            && target.matches(trigger.getTrigger()[0])) {
                        try {
                            trigger.getMethod()
                                    .invoke(trigger.getHostClass()
                                            .newInstance(), event);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        ...
                        && event.getClass() != RequestEvent.class) {
                    if (trigger.getMethod().getParameterTypes()[0]
                            .isInstance(event)) {
                        try {
                            trigger.getMethod()
